...
local function mark(win, role)
  vim.w[win].claude_agents_ui = role
...
function M.build_chat_column(thread)
  ...
  local chat_buf = require("claude-agents.ui.chat").ensure_buf(thread)
  local input_buf = require("claude-agents.ui.input").ensure_buf(thread)
...
local function build_tab(thread)
  ...
  if thread.layout then
    require("claude-agents.persist.layout").restore(thread.layout)
  end
...
function M.open(thread)
  ...
  local focus = require("claude-agents.config").options.ui.focus_on_open
  if focus == "input" then
...
function M.toggle_chat()
  ...
  if not thread then
    vim.notify("claude-agents: current tab is not a thread workspace", vim.log.levels.WARN)
    return
  end
...
function M.capture_layout(thread)
  ...
  local layout = require("claude-agents.persist.layout").capture(thread.tabpage)
  if layout then
