...
local ns = vim.api.nvim_create_namespace("claude-agents-chat")

local kind_hl = {
  user = "ClaudeAgentsChatUser",
  tool = "ClaudeAgentsChatTool",
  thinking = "ClaudeAgentsChatThinking",
  meta = "ClaudeAgentsChatMeta",
  error = "ClaudeAgentsChatError",
  permission = "ClaudeAgentsChatPermission",
}
...
local function render_entry(buf, kind, text)
  ...
  elseif kind == "meta" then
    for i = 0, #lines - 1 do
      vim.api.nvim_buf_set_extmark(buf, ns, start + i, 0, { line_hl_group = "ClaudeAgentsChatMeta", priority = 90 })
    end
  end
  if kind == "meta" then
    ...
    for i, l in ipairs(lines) do
      if l:sub(1, 2) == "+ " then
        vim.api.nvim_buf_set_extmark(buf, ns, start + i - 1, 0, { line_hl_group = "ClaudeAgentsDiffAdd", priority = 95 })
      elseif l:sub(1, 2) == "- " then
        vim.api.nvim_buf_set_extmark(buf, ns, start + i - 1, 0, { line_hl_group = "ClaudeAgentsDiffDelete", priority = 95 })
      end
    end
  end
end
...
function M.append(thread, kind, text)
  ...
  autoscroll(buf)
  require("claude-agents.persist.store").save_debounced()
end
