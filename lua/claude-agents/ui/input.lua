...
local function send(thread)
  ...
  require("claude-agents.agent.session").get(thread):send(text)
end
...
function M.ensure_buf(thread)
  ...
  vim.api.nvim_buf_set_name(buf, "claude-agents://input/" .. thread.slug .. "/" .. thread.id)
...
function M.focus(thread)
  ...
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(thread.tabpage)) do
    if vim.w[win].claude_agents_ui == "input" then
