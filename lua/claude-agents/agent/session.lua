local process = require("claude-agents.agent.process")
...
local function chat()
  return require("claude-agents.ui.chat")
end

function Session:build_args()
  local cfg = require("claude-agents.config").options.claude
  local args = {
    cfg.cmd,
    "-p",
    "--input-format",
...
function Session:on_content_block(block)
  local ui = require("claude-agents.config").options.ui
...
function Session:on_result(event)
  self.busy = false
  local cfg = require("claude-agents.config").options
...
function Session:on_control_request(event)
  ...
  if request.subtype == "can_use_tool" then
    ...
    require("claude-agents.ui.permission").show(self.thread, self.pending_permission)
    ...
    process.send(self.job, {
      type = "control_response",
      response = { subtype = "error", request_id = event.request_id, error = "unsupported by claude-agents.nvim" },
    })
  end
end
...
function Session:on_line(line)
  ...
  if event.type == "system" and event.subtype == "init" then
    if event.session_id then
      ...
      require("claude-agents.core.registry").emit("state")
    end
      ...
      for _, block in ipairs(content) do
        if block.type == "tool_result" and block.is_error then
          ...
          if type(text) == "string" and text ~= "" then
            chat().append(self.thread, "error", require("claude-agents.util").shorten(text, 200))
          end
        end
      end
    end
...
function Session:schedule_reap()
  local timeout = require("claude-agents.config").options.claude.idle_timeout
